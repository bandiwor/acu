...
AcuConstVal AcuCodeGen_EvalConst(AcuWorkspace *ws, AstNodeIdx expr_idx) {
    ...
    switch (node->type) {
        case AST_LITERAL: {
            ...
            TypeId node_type = V_At(&ws->node_types, expr_idx);

            if (TypePrimitiveKind_IsFloat(node_type) || lit->kind == ACU_LITERAL_F64) {
                f64 fval =
                    (lit->kind == ACU_LITERAL_F64)
                        ? lit->as.f64
                        : (lit->kind == ACU_LITERAL_I64 ? (f64)lit->as.i64 : (f64)lit->as.u64);
                return (AcuConstVal){.kind = CONST_FLOAT, .as.f64_val = fval};
            }
        ...
        case AST_UNARY: {
            if (node->as.unary.type == AST_UNARY_PLUS) {
                return AcuCodeGen_EvalConst(ws, node->as.unary.right);
            }
            ...
            if (node->as.unary.type == AST_UNARY_MINUS) {
                if (r.kind == CONST_INT) {
                    if (r.as.i64_val == (i64)0x8000000000000000ULL) {
                        return (AcuConstVal){.kind = CONST_NONE};
                    }
        ...
        case AST_BINARY: {
            ...
            if (l.kind == CONST_BOOL && r.kind == CONST_BOOL) {
                bool a = l.as.bool_val;
                bool b = r.as.bool_val;
            ...
            if (l.kind == CONST_INT && r.kind == CONST_INT) {
                ...
                switch (node->as.binary.type) {
                    case AST_BINARY_ADD:
                        return (AcuConstVal){.kind = CONST_INT,
                                             .as.i64_val = (i64)((u64)a + (u64)b)};
                    case AST_BINARY_SUB:
                        return (AcuConstVal){.kind = CONST_INT,
                                             .as.i64_val = (i64)((u64)a - (u64)b)};
                    case AST_BINARY_MUL:
                    ...
                    case AST_BINARY_SHR: {
                        if (b < 0 || b >= 64)
                            return (AcuConstVal){.kind = CONST_NONE};
                        TypeId left_t = V_At(&ws->node_types, node->as.binary.left);
                        if (TypePrimitiveKind_IsUnsignedInt(left_t)) {
                            return (AcuConstVal){.kind = CONST_INT,
                                                 .as.i64_val = (i64)((u64)a >> b)};
                        }
                        return (AcuConstVal){.kind = CONST_INT, .as.i64_val = a >> b};
                    }
                    case AST_BINARY_BITWISE_AND:
                ...
                switch (node->as.binary.type) {
                    ...
                    case AST_BINARY_DIV:
                        return (AcuConstVal){.kind = CONST_FLOAT, .as.f64_val = fa / fb};
                    case AST_BINARY_REM:
                        return (AcuConstVal){.kind = CONST_FLOAT, .as.f64_val = fmod(fa, fb)};
                    case AST_BINARY_POW:
                        return (AcuConstVal){.kind = CONST_FLOAT, .as.f64_val = pow(fa, fb)};
                    case AST_BINARY_EQ:
        ...
        case AST_TYPE_CAST: {
            ...
            if (TypePrimitiveKind_IsInteger(target_type)) {
                i64 int_val = (val.kind == CONST_FLOAT) ? (i64)val.as.f64_val : val.as.i64_val;
                u32 bits = TypePrimitiveKind_GetIntBitWidth(target_type);
                bool is_signed = TypePrimitiveKind_IsSignedInt(target_type);

                if (bits < 64) {
                    u64 mask = (1ULL << bits) - 1;
                    u64 masked = ((u64)int_val) & mask;
                    if (is_signed && (masked & (1ULL << (bits - 1)))) {
                        int_val = (i64)(masked | ~mask);
                    } else {
                        int_val = (i64)masked;
                    }
                }
                return (AcuConstVal){.kind = CONST_INT, .as.i64_val = int_val};
            }
            return val;
        }
        ...
        case AST_CALL: {
            ...
            for (u32 i = 0; i < arg_count; ++i) {
                ...
                if (TypePrimitiveKind_IsFloat(arg_type)) {
                    f64 fval = (cv.kind == CONST_FLOAT) ? cv.as.f64_val : (f64)cv.as.i64_val;
                    if (arg_type == TYPE_PRIMITIVE_F32) {
                        args[i].f32 = (f32)fval;
                    } else {
                        args[i].f64 = fval;
                    }
                } else if (TypePrimitiveKind_IsInteger(arg_type)) {
                    i64 ival = (cv.kind == CONST_INT) ? cv.as.i64_val : (i64)cv.as.f64_val;
                    args[i].i64 = ival;
...
bool AcuCodeGen_IsPure(AcuWorkspace *ws, AstNodeIdx idx) {
    ...
    switch (n->type) {
        ...
        case AST_DOT:
            return AcuCodeGen_IsPure(ws, n->as.dot.object);
        ...
        case AST_WHILE: {
            AcuConstVal cond_const = AcuCodeGen_EvalConst(ws, n->as.while_stmt.condition);
            if (cond_const.kind == CONST_BOOL && cond_const.as.bool_val == false) {
